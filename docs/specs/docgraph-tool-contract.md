...
Use `docgraph_read_dg_search` for exact names, aliases, symbols, and quick lexical/semantic lookup. Use `docgraph_read_dg_context` for compact role-aware packets. Start with `budget=small`; use `mode=hybrid` or `mode=mix` only when exact/small lookup leaves material ambiguity.
...
docgraph_write_dg_stale_scan(auto_ingest=false)
```
...
run small search/context checks if cross-role impact is possible
...
`docgraph_write_dg_mutation_schema` is compact by default. Use `detail="full"` only for examples and debugging.

`docgraph_write_dg_validate` is compact by default and returns counts plus the first `limit` errors/warnings. Use `detail="full"` only for debugging.
...
`docgraph_write_dg_commit_update` returns compact validation failures by default. Increase `error_limit` only when more failing rows are needed.

`docgraph_write_dg_stale_scan` is dry-run by default and returns counts plus the first `result_limit` entries. If `auto_ingest=true`, pass `source_id` or `uri` for targeted repair when possible. Broad auto-ingest is capped by `max_sources` and should be raised only deliberately. Use `detail="full"` only for debugging.
