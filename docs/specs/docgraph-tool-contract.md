...
`docgraph_write_dg_stale_scan` is dry-run by default. If `auto_ingest=true`, pass `source_id` or `uri` for targeted repair when possible. Broad auto-ingest is capped by `max_sources` and should be raised only deliberately.
