...
class DocGraphBackend:
    ...
    def __init__(self, db_path: str | Path, root: str | Path | None = None, config_path: str | Path | None = None):
        ...
        self._source_ingest = SourceIngestFlow(
            self,
            id_factory=new_id,
            ts_factory=now_ts,
            text_hash=sha256_text,
        )
        self._relink_flow = RelinkFlow(
            self,
            fts_query=fts_query,
            text_hash=sha256_text,
        )
    ...
    def _safe_resolve_uri(self, uri: str) -> Path:
        return self._source_ingest.safe_resolve_uri(uri)

    def _safe_read_uri(self, uri: str) -> str:
        return self._source_ingest.safe_read_uri(uri)
    ...
    def ingest_source(
        ...
        return self._source_ingest.ingest_source(
    ...
    def _chunk_refs_for_source(self, source_id: str, preview_chars: int = 180) -> list[dict[str, Any]]:
        return self._source_ingest.chunk_refs_for_source(source_id, preview_chars=preview_chars)

    def suggest_evidence_relinks(self, claim_id: str, limit: int = 10) -> dict[str, Any]:
        return self._relink_flow.suggest_evidence_relinks(claim_id=claim_id, limit=limit)
    ...
    def suggest_source_relinks(
        ...
        return self._relink_flow.suggest_source_relinks(
            source_id=source_id,
            uri=uri,
